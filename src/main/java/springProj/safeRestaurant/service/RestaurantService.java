...
public class RestaurantService {
    ...
    public List<Restaurant> RestrLisetFoundByAddress(String keyword) throws Exception {
        List<Restaurant> list = this.getRestrList();
        return list.stream().filter(r ->r.getJibeon().contains(keyword)).collect(Collectors.toList());
    }
    public List<Restaurant> RestrLisetFoundByName(String keyword) throws Exception {
        List<Restaurant> list = this.getRestrList();
        return list.stream().filter(r ->r.getName().contains(keyword)).collect(Collectors.toList());
    }

    public Restaurant get(Long restrNum) throws Exception {
        List<Restaurant> list = this.getRestrList();
        return list.stream().filter(r -> r.getSafetyNo().equals(restrNum)).findFirst().orElse(null);
    }

    public List<Restaurant> getRestrList() throws Exception { //파싱 받은 JSONArray(레스토랑 목록)를 레스토랑 객체 리스트로 만들어줌
